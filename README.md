# LightHouse ![LightHouse](https://www.readmecodegen.com/api/social-icon?name=LightHouse&size=64&bg=%23f3f4f6&shape=circle&color=%233b82f6)
...
```sh
make docker-build docker-push IMG=<some-registry>/lighthouse:tag
```
...
```sh
make deploy IMG=<some-registry>/lighthouse:tag
```
...
```sh
make build-installer IMG=<some-registry>/lighthouse:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/lighthouse/<tag or branch>/dist/install.yaml
```
